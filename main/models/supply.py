...
#    super(Supply, self).save(*args, **kwargs)
  ...
  def wrightOff(self):
    if self.status == self.WEIGHTED \
        and reduce(lambda a, b: a and b, map(lambda a: a.order.status == SupplyOrder.WHEIGHTED or a.order.status == SupplyOrder.DELIVERED, self.supplyorderitem_set.all()), True):
    ...
    if self.isPartnerGood():
      ...
      for goodtogood in self.partnerGood.partnergoodtogood_set.all():
        ret += goodtogood.good.name + ': ' \
          + str(sum(map(lambda x: x.value, self.supply.supplyorderitem_set.filter(good_id=goodtogood.good.pk)))) + '<BR />'
      return ret
